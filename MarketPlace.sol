...
contract NFTShellMarketplace is Ownable {
  uint public offerCount;
  using SafeMath for uint256;
  mapping (uint => _Offer) public offers;
  mapping (address => uint) public userFunds;
  mapping (uint256 => uint256) public orderByNFT;
  ...
  address public nftShellWallet;
  uint256 public COMISSION_RATE = 25; //2.5%
  ...
  constructor(address _nftCollection, address _nftShellWallet) {
    nftCollection = IERC721(_nftCollection);
    nftShellWallet = _nftShellWallet;
  }


  function setNFTShellWallet(address _nftShellWallet) public onlyOwner{
      nftShellWallet = _nftShellWallet;
  }

  
  function listNFT(uint _id, uint _price) public {
    nftCollection.transferFrom(msg.sender, address(this), _id);
    offerCount ++;
    offers[offerCount] = _Offer(offerCount, _id, msg.sender, _price, false, false);
    orderByNFT[_id] = offerCount;
  ...
  function fillOffer(uint _offerId) public payable {
    ...
    uint256 comission = msg.value.mul(COMISSION_RATE).div(1000);
    payable(nftShellWallet).transfer(comission);
    userFunds[_offer.user] += msg.value.sub(comission);
    orderByNFT[_offer.id] = 0;
  ...
  function getOfferByNFT(uint256 nftId ) public view returns(_Offer memory offer){
      uint256 orderId = orderByNFT[nftId];
